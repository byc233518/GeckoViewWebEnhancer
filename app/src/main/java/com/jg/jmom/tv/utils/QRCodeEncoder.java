...
public class QRCodeEncoder {
     ...
     */
    public static Bitmap syncEncodeQRCode(Context context,String content, int size, int foregroundColor, int backgroundColor, Bitmap logo ,int border , int borderColor) {
        int borderWidth= (int) dp2px(context,4);
        size-=borderWidth*2;
        try {
            BitMatrix matrix = new MultiFormatWriter().encode(content, BarcodeFormat.QR_CODE, size, size, HINTS);
            int[] pixels = new int[size * size];
            ...
            bitmap=addBorderToQRCode(context,bitmap,backgroundColor ,border,borderColor);
     ...
     * 添加logo到二维码图片上
     */
    private static Bitmap addBorderToQRCode(Context context,Bitmap src, int backgroundColor, int border , int borderColor) {
        if (src == null) {
            return src;
        }
